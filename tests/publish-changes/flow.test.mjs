...
function createHarness({
  ...
  const gh = {
    ...
    listPullRequests: async (_repo, args) => {
      if (args.includes('--head') && args.includes('--state') && args.includes('open') && prCreated) {
        return [{ number: 12 }];
      }
      return [];
    },
...
describe('publish flow classification gates', () => {
  ...
  it.each(['normal', 'significant'])(
    'keeps %s updates at PR-only by default',
  ...
  it.each(['normal', 'significant'])(
    'verifies %s auto-merge before refreshing when policy explicitly allows it',
    async (classification) => {
  ...
  it('reports confirmed files, documentation, memory, validation, and external actions', async () => {
    ...
    harness.git.statusZ = async () =>
      ' M README.md\0 M .codex/project/project-guideline.md\0';
    harness.git.diff = async (args) => {
