...
PUBLISH_IMPLEMENTATION="$REPO_ROOT/kit/scripts/publish-changes.sh"
WORKFLOW_COMMON="$REPO_ROOT/kit/scripts/lib/workflow-common.sh"
SOURCE_WORKFLOW_COMMON="$REPO_ROOT/scripts/lib/workflow-common.sh"
TEST_ROOT=""
PASS_COUNT=0
...
run_implementation_case_without_message() {
  local case_root="$1"
  local input="$2"
  local output="$3"
  ...
  printf '%s' "$input" | env \
    ...
    PUBLISH_TEST_STATE="$case_root/state" \
    PUBLISH_TEST_REPO="$case_root/repo" \
    bash "$PUBLISH_IMPLEMENTATION" > "$output" 2>&1
}
...
test_source_wrapper_delegates_to_installable_implementation() {
  assert_contains "$PUBLISHER" "kit/scripts/publish-changes.sh"
  assert_not_contains "$PUBLISHER" "classify_update()"
  assert_contains "$SOURCE_WORKFLOW_COMMON" "kit/scripts/lib/workflow-common.sh"
  assert_not_contains "$SOURCE_WORKFLOW_COMMON" "confirm()"

  log_pass "source-repository wrappers delegate to the installable implementation and helper"
}

test_installable_implementation_runs_directly() {
  local case_root output
  case_root="$(new_case installable-no-op)"
  output="$case_root/output.log"
  touch "$case_root/state/clean" "$case_root/state/no-unpushed"

  run_implementation_case_without_message "$case_root" '' "$output"
  ...
  log_pass "installable publish implementation runs directly without network access"
}
...
test_existing_pr_gets_publish_record() {
  ...
  assert_contains "$case_root/state/commands.log" 'Generated by `publish-changes.sh`.'
  assert_not_contains "$case_root/state/commands.log" "gh pr create"
...
main() {
  ...
  bash -n "$PUBLISHER" "$PUBLISH_IMPLEMENTATION" "$WORKFLOW_COMMON" "$SOURCE_WORKFLOW_COMMON"
